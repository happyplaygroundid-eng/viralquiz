...
# IMPORT MOVIEPY STANDAR (MENGGUNAKAN moviepy==1.0.3)
...
st.title("💀 Viral Shorts Factory (V1.0.3 Stable)")
    ...
    if not bg_video or not font_file or selected_row is None:
        ...
        with st.spinner("Rendering..."):
            try:
                ...
                # 2. Background Video (KEMBALI KE .subclip)
                ...
                # 4. Audio Mixing (KEMBALI KE .set_start / .set_duration)
                ...
                if timer_sfx:
                    ...
                    audio_clips.append(AudioFileClip(s_p).set_start(11).set_duration(5))
                
                final_audio = CompositeAudioClip(audio_clips).set_duration(26)
                final_video = CompositeVideoClip([clip_bg, c1, c2, c3, c4, c5, c6]).set_audio(final_audio)
