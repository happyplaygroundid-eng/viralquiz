...
st.title("💀 Viral Shorts Factory (V2.0 Fix)")
...
if st.button("🚀 MULAI GENERATE VIDEO", use_container_width=True):
    if not bg_video or not font_file or selected_row is None:
        ...
        with st.spinner(f"Rendering video #{q_number}..."):
            try:
                ...
                # 2. Background (FIX V2 SYNTAX)
                bg_path = os.path.join(tmp_dir, "b.mp4")
                with open(bg_path, "wb") as f: f.write(bg_video.read())
                
                clip_bg = VideoFileClip(bg_path)
                # Gunakan .subclip() atau .cut() tergantung versi
                clip_bg = clip_bg.cut(0, 26) if hasattr(clip_bg, 'cut') else clip_bg.subclip(0, 26)
                
                # Resize fix
                if hasattr(clip_bg, 'resized'): clip_bg = clip_bg.resized(height=1920)
                else: clip_bg = clip_bg.resize(height=1920)
                ...
                download_placeholder.download_button(label=f"📥 DOWNLOAD: {clean_name}", data=final_bytes, file_name=clean_name, mime="video/mp4", type="primary", use_container_width=True)
